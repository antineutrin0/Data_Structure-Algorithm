...
void subArraySum(vector<int>& v, int num, int sum, int n, vector<int>& vis, int i) {
    if (i == n) {
        if (sum == num) {
            res.push_back(vis);
        }
    ...
    if (sum + v[i] <= num) {
        vis[i] = 1;
        subArraySum(v, num, sum + v[i], n, vis, i + 1);
        vis[i] = 0;
    }

    subArraySum(v, num, sum, n, vis, i + 1);
    return;
}
...
int32_t main() {
    ...
    int t=1;//cin>>t;
    while (t--) {
    	vector<int>v;int n,num;cin>>n>>num;
    	vector<int>vis(n,0);
    	for(int i=0;i<n;i++)
    	{
